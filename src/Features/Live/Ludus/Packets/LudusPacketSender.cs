...
namespace ScoreSaber.Features.Live.Ludus.Packets {
    internal sealed class LudusPacketSender {
        ...
        internal LudusPacketSender(Action<byte[]> send, Func<Func<byte[]>, bool> sendDeferred) {
            _send = send;
        ...
        internal bool ReplayPacket(ReplayStreamPacket packet, string connectionId) {
            ...
            if (!_sendDeferred(() => LudusProto.EncodeReplayPacket(packet, sequence, connectionId))) {
                return false;
            }
